...
const Login: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit(handleSubmitForm)} className='flex justify-center items-center min-h-screen py-5'>
      <div className='w-[40%] shadow-md border rounded-2xl px-5 py-8'>
        ...
        <div className='flex flex-col gap-2 mb-1 mt-4'>
          <div className='flex flex-col mb-3 space-y-1'>
            ...
            <input {...register('email')} type='text' placeholder='Email or username' className='focus:outline-none rounded-xl bg-slate-100 px-3 py-2 border text-sm focus:ring-2' />
          </div>
          <div className='flex flex-col mb-3 space-y-1 relative'>
            {/* <label className='text-sm font-semibold pl-3'>
              Password
            </label> */}
            <input {...register('password')} type={showPassword ? 'text' : 'password'} placeholder='Password' className='focus:outline-none rounded-xl bg-slate-100 px-3 py-2 border text-sm focus:ring-2' />
            {
              showPassword ? (
        ...
        <div className='flex items-center justify-end hover:cursor-pointer mb-3'>
          <Link to={'reset-password'} className='text-sm font-normal text-blue-700 hover:cursor-pointer hover:underline hover:underline-offset-1' >Forgot password</Link>
        </div>
        <div className='space-y-4'>
          <button disabled={isPending} className={`flex justify-center items-center w-full bg-blue-600 px-1 py-[10px] text-white font-medium rounded-xl focus:ring-2 ${isPending ? 'cursor-wait bg-blue-400' : ''}`}>
            {isPending ? (
              <>
                <Spinner size={15} />
                <p>Loading...</p>
          ...
          <button className='w-[100%] border-2 font-medium rounded-xl py-[8px] flex gap-2 justify-center items-center text-sm'>
